...
## Digital Friction MVP 新增运行说明

### 1) LLM 主判场景匹配（规则兜底）

- `MATCH_PIPELINE_MODE=llm_unified_primary`（当前仅支持该值）
- `LLM_SCENARIO_MIN_CONFIDENCE`：LLM 匹配最低置信阈值
- `LLM_SCENARIO_MAX_CALLS_PER_STEP`：单 step 场景匹配调用上限

审计字段会写入 `mvp_decision_attempt`：
- `scenario_match_source`（`llm_primary/rule_fallback/random_fallback`）
- `llm_match_status`
- `llm_match_confidence`
- `llm_match_reason`

> 注：`SCENARIO_MATCH_MODE` 已移除，不再生效。

### 2) 事件解释向量与阶段解释汇总

每次决策 attempt 会新增解释字段：
- `reason_primary`
- `reason_secondary`
- `reason_tags_json`
- `reason_weights_json`
...
阶段结算会生成：
- SQLite 表：`as_<exp_id>_mvp_stage_explanation`
- CSV：`examples/digital_friction_mvp/analysis/stage_explanation_<exp8>.csv`
- 同时把每个 agent 的阶段摘要写入 stream memory，topic 为 `digital_friction_stage_summary`。
...
seed 规则（严格配对）：
- 同一 `pair_seed` 下，A/B/C 三世界使用同一个 seed（不再是 `base_seed + world_index`）。
- 多组配对可通过 `--n-seeds`（从 `base_seed` 递增）或 `--seed-list`（显式列表）控制。

示例（显式 seed 列表）：

```bash
python examples/digital_friction_mvp/world_runner.py \
  --group-id demo_group_seedlist \
  --seed-list 101,202,303,404 \
  --summarize
```

在 `--summarize` 之后，推荐追加 paired 差值统计：

```bash
python examples/digital_friction_mvp/analysis_parallel_paired.py \
  --summary-csv examples/digital_friction_mvp/analysis/parallel_world_summary_demo_group.csv
```

输出：
- `examples/digital_friction_mvp/analysis/exp_group_manifest_<group>.json`
- `examples/digital_friction_mvp/analysis/exp_group_manifest_<group>.csv`
- `examples/digital_friction_mvp/analysis/config_snapshot_<group>.json`（固定参数指纹快照）
- `examples/digital_friction_mvp/analysis/parallel_world_summary_<group>.csv`（开启 `--summarize` 时）
- `examples/digital_friction_mvp/analysis/parallel_world_paired_diffs.csv`（paired 分组差值）
- `examples/digital_friction_mvp/analysis/parallel_world_paired_stats.csv`（均值/CI/Wilcoxon/方向一致性）
- `examples/digital_friction_mvp/analysis/parallel_world_paired_qc.csv`（QC 纳入与剔除原因）

### 4) 运行元数据文件

可通过环境变量指定：

- `RUN_METADATA_PATH=/path/to/run_meta.json`

`main.py` 在实验结束时写出元数据（`exp_id/world/seed/status` 等），供批量编排脚本回填 manifest。  
新增字段包含：`pair_index/pair_seed/world_order/config_fingerprint`、`opportunities_total` 及关键运行开关，便于后续严格配对分析与 QC。
