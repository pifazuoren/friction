...
class VectorStore:
    ...
    def _search_points(
        ...
        def _call(method_name: str, call_kwargs: dict):
            ...
            try:
                ...
                if "query_filter" in call_kwargs:
                    adjusted = dict(call_kwargs)
                    adjusted["filter"] = adjusted.pop("query_filter")
                    call_kwargs = adjusted
                if method_name == "query_points" and "query_vector" in call_kwargs:
                    adjusted = dict(call_kwargs)
                    adjusted["query"] = adjusted.pop("query_vector")
                    call_kwargs = adjusted
                try:
                    return method(**call_kwargs)
                except AssertionError:
    ...
    async def similarity_search(
        ...
        search_result = self._search_points(named_vector, k, must_conditions)
