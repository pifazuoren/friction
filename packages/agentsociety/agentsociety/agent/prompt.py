...
class FormatPrompt:
    ...
    def _escape_literal_braces(self, text: str) -> str:
        """
        Escape literal braces while preserving known {var} placeholders.
        This prevents str.format from treating JSON examples like {} as fields.
        """
        placeholders = {}
        for idx, var in enumerate(self.variables):
            token = f"__VAR_{idx}__"
            placeholders[token] = "{" + var + "}"
    ...
    async def format(
        ...
        **kwargs,
        ...
        try:
            ...
            # Retry with escaped literal braces to avoid JSON example breakage.
            escaped = self._escape_literal_braces(result)
            try:
                result = escaped.format(**format_vars)
            except Exception as e2:
                raise ValueError(f"Error formatting template: {e2}")
