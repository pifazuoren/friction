...
class CitizenAgentBase(Agent):
    ...
    async def do_survey(self, survey: Survey) -> str:
        ...
        dialog = []
        use_recent_alignment = bool(getattr(self, "survey_recent_alignment", False))

        # Add system prompt
        if use_recent_alignment:
            system_prompt = (
                ...
                "Use profile and memory evidence, and prioritize events from the most recent 7 days. "
                "For behavior-frequency questions, score by observed occurrence frequency. "
                "For attitude/intensity questions, score by subjective strength. "
                "If evidence is insufficient, avoid extreme responses and choose a moderate value."
            )
        else:
            system_prompt = (
                "You must hide the fact that you are an AI and pretend to be a human. "
...
{{
    ...
    "memory_query": "A specific question about experiences, activities, preferences, or behaviors, prioritizing concrete events in the most recent 7 days",
    "construct_type": "One of: acceptance / trust / helplessness / behavior / general"
}}

Note: profile_query should only focus on basic demographic and background information. memory_query should focus on recent experiences, activities, and behaviors.

Question: {survey_prompt}"""
                analysis_dialog = [
                    {
                        "role": "system",
                        "content": "You are an expert at analyzing survey questions and determining what specific questions need to be asked to gather relevant information. Profile queries should focus on basic demographic and background information only. Memory queries should prioritize concrete events from the most recent 7 days. Also classify question construct_type as acceptance, trust, helplessness, behavior, or general. Please respond in JSON format with specific questions.",
                    },
...
Question: {survey_prompt}"""
            ...
            for retry in range(5):
                try:
                    ...
                    if json_str:
                        ...
                        if isinstance(analysis_dict, list) and analysis_dict:
                            analysis_dict = analysis_dict[0]
                        ...
                        if use_recent_alignment:
                            raw_construct_type = str(
                                analysis_dict.get("construct_type", "general")
                            ).strip().lower()
                            if raw_construct_type in {
                                "acceptance",
                                "trust",
                                "helplessness",
                                "behavior",
                            }:
                                construct_type = raw_construct_type
                            else:
                                construct_type = "general"
                        break
            ...
            if use_recent_alignment:
                construct_guidance_map = {
                    "acceptance": "Focus on willingness and perceived usefulness/ease in real use.",
                    "trust": "Focus on confidence in app reliability, safety, and correctness.",
                    "helplessness": "Focus on control loss, expected failure, and low self-efficacy.",
                    "behavior": "Focus on observed frequency of delay, proxy reliance, or switching offline.",
                    "general": "Focus on evidence-consistent and balanced judgment.",
                }
                construct_guidance = construct_guidance_map.get(
                    construct_type, construct_guidance_map["general"]
                )
                dialog.append(
                    {
                        "role": "system",
                        "content": (
                            "Answer the survey question based on following information.\n"
                            "Scoring rules:\n"
                            "1) Prioritize concrete events from the most recent 7 days.\n"
                            "2) If question is behavior-related, score by event frequency.\n"
                            "3) If question is attitude-related, score by subjective intensity.\n"
                            "4) Keep score direction consistent with evidence (more failures -> higher helplessness/avoidance).\n"
                            "5) If evidence is limited, avoid extreme values and return a moderate score.\n"
                            "6) Keep output as valid JSON only.\n\n"
                            f"Question construct_type: {construct_type}\n"
                            f"Construct guidance: {construct_guidance}\n\n"
                            f"My background story: {background_story}\n\n"
                            f"My Profile:\n{profile_and_states}\n\n"
                            f"My Related Memory:\n{relevant_memory}"
                        ),
                    }
