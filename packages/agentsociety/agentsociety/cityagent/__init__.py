...
def _is_citizen_like_agent_class(agent_class: object) -> bool:
    if agent_class == "citizen":
        return True
    if isinstance(agent_class, type):
        try:
            return issubclass(agent_class, SocietyAgent)
        except TypeError:
            return False
    return False


def _has_economy_block_config(agent_config: AgentConfig) -> bool:
    if agent_config.agent_class == "citizen":
        return True
    if agent_config.blocks is None:
        return False
    for key in agent_config.blocks:
        if key == "EconomyBlock" or key == EconomyBlock:
            return True
    return False
...
def default(config: Config) -> Config:
    ...
    # =====================
    if any(
        _is_citizen_like_agent_class(citizen_config.agent_class)
        and _has_economy_block_config(citizen_config)
