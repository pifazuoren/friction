...
class CognitionBlock(Block):
    ...
    async def thought_update(self):
        ...
        if incident_str:
            safe_incident_str = (
                str(incident_str).replace("{", "{{").replace("}", "}}")
            )
            incident_prompt = "Today, these incidents happened:\n" + safe_incident_str
        else:
            incident_prompt = "No incidents happened today."
        ...
        question_prompt = FormatPrompt(question_prompt)
        emotion = await self.memory.status.get("emotion")
        sadness = int(emotion.get("sadness", 5))
        joy = int(emotion.get("joy", 5))
        fear = int(emotion.get("fear", 5))
        disgust = int(emotion.get("disgust", 5))
        anger = int(emotion.get("anger", 5))
        surprise = int(emotion.get("surprise", 5))
        raw_thought = await self.memory.status.get("thought")
        safe_thought = str(raw_thought).replace("{", "{{").replace("}", "}}")
        ...
        for retry in range(10):
            try:
                ...
                if json_str:
                    ...
                    if isinstance(response, dict) and response.get("thought"):
                        evaluation = False
                        break
            except Exception:
                pass
        ...
        thought_value = response.get("thought")
        if not thought_value:
            thought_value = await self.memory.status.get(
                "thought", default_value="I am not sure what to think."
            )
    ...
    async def emotion_update(self, incident):
        ...
        incident_prompt = str(incident).replace("{", "{{").replace("}", "}}")
        question_prompt = """
        ...
        updated_emotion = {
            "sadness": int(response.get("sadness", sadness)),
            "joy": int(response.get("joy", joy)),
            "fear": int(response.get("fear", fear)),
            "disgust": int(response.get("disgust", disgust)),
            "anger": int(response.get("anger", anger)),
            "surprise": int(response.get("surprise", surprise)),
        }
        await self.memory.status.update("emotion", updated_emotion)
        await self.memory.status.update(
            "emotion_types", str(response.get("word", "Unknown"))
        )
        return str(response.get("conclusion", ""))
