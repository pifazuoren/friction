...
def _extract_place_type(payload) -> str:
    if isinstance(payload, dict):
        return str(payload.get("place_type", "")).strip()
    if isinstance(payload, list) and payload:
        return _extract_place_type(payload[0])
    if payload is None:
        return ""
    return str(payload).strip()


def _normalize_place_type(place_type: str, options: list[str]) -> str:
    if not options:
        return ""
    if not place_type:
        return random.choice(options)
    if place_type in options:
        return place_type
    ...
    for option in options:
        if lowered in option.lower() or option.lower() in lowered:
            return option
    synonyms = {
        "shopping": ["shop", "mall", "retail", "market", "commercial", "购物", "商场", "超市"],
        "restaurant": ["food", "eat", "dining", "restaurant", "餐", "饭"],
        "work": ["office", "company", "workplace", "办公", "公司"],
        "home": ["home", "house", "residential", "apartment", "住宅", "居住"],
        "hospital": ["hospital", "clinic", "medical", "医院", "诊所"],
        "park": ["park", "green", "公园", "绿地"],
        "school": ["school", "university", "campus", "学校"],
        "bank": ["bank", "finance", "银行"],
        "transport": ["station", "airport", "bus", "metro", "rail", "交通", "车站", "地铁"],
    }
    for key, tokens in synonyms.items():
        if key in lowered:
            for option in options:
                if any(token in option.lower() or token in option for token in tokens):
                    return option
...
class PlaceSelectionBlock(Block):
    ...
    async def forward(self, context: DotDict):
        ...
        try:
            ...
            parsed = json_repair.loads(clean_json_response(levelOneType))
            levelOneType = _normalize_place_type(
                _extract_place_type(parsed), list(poi_cate.keys())
            )
            sub_category = poi_cate.get(levelOneType)
            if not sub_category:
                raise KeyError(levelOneType)
        ...
        try:
            ...
            parsed = json_repair.loads(clean_json_response(levelTwoType))
            levelTwoType = _normalize_place_type(
                _extract_place_type(parsed), sub_category
            )
        except Exception as e:
